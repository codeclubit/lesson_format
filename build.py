...
def build_project(rebuild, term, project, language, theme, root_dir, output_dir, breadcrumb):
    ...
    if project.materials:
        #copy the generated pdf into input directory. quite some workaround and shortcust.
        _pdf_name, _pdf_ext = os.path.basename(input_file).rsplit(".", 1)
        pdf_name = os.path.join(output_dir, _pdf_name + ".pdf")
        _pdf_note_name, _pdf_note_ext = os.path.basename(input_file).rsplit(".", 1)
        pdf_note_name = os.path.join(output_dir, _pdf_note_name + "-Note.pdf")
        copy_file(pdf_name, os.path.dirname(input_file))
        copy_file(pdf_note_name, os.path.dirname(input_file))
        time.sleep(3)
        project.materials.append(os.path.join(os.path.dirname(input_file),  _pdf_name + ".pdf"))
        project.materials.append(os.path.join(os.path.dirname(input_file),  _pdf_note_name + "-Note.pdf"))
